									...
									<?php foreach ($this->m->get_level() as $level) : ?>
										<?php if (array_search($this->session->userdata('id'), explode(",", $level->level_show_landing))) : ?>
											<div class="col-lg-3 col-6">
												<a href="<?= base_url('dashboard/landing/akses/' . $level->level_id . '?url=' . $level->level_url) ?>" data="<?= $level->level_id ?>" class="text-light">
													<div class="small-box p-3" style="background-color: <?= $level->level_background ?>">
