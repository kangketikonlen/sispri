...
<script>
	$(document).ready(function() {
		var url = "<?= base_url('indikator/bor/get_jumlah?') ?>";
		var urlChart = "<?= base_url('indikator/bor/chart_data?') ?>";
		var urlTable = "<?= base_url('indikator/bor/get_table?') ?>";
		// 
				...
				$.each(opts, function(key, dt) {
					...
					for (let i = 0; i < dt.length; i++) {
						$("#dtTable tbody").append(
							"<tr>" +
							"<td>" + dt[i].kelas + "</td>" +
							"<td class='text-right'>" + number_format(dt[i].kapasitas) + " bed</td>" +
							"<td class='text-right'>" + number_format(dt[i].periode) + " hari</td>" +
							"</tr>"
						);
					}
				});
